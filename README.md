...
npm install "https://github.com/o16s/rules-editor/archive/refs/tags/v0.4.2.tar.gz"
```
...
```bash
npm install github:o16s/rules-editor#v0.4.2
```
...
The git tag is `rules-engine/v0.4.2`, because the module sits in a
subdirectory. Go maps the path suffix to that tag prefix by itself, so the
version string stays `v0.4.2`.
