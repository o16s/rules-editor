...
import { resolveCatalog, type EditorState } from './state.js';

export interface MenuDeps {
  root: HTMLElement;
  state: EditorState;
  /** Where the menu goes for the bar's input: inside the bar, above its input line. Null for a cell input. */
  inline: (input: HTMLInputElement) => { container: HTMLElement; before: HTMLElement } | null;
...
export function createMenu(deps: MenuDeps): Menu {
  ...
  function maybe(input: HTMLInputElement): void {
    // On a phone the bar is the only place to type; a cell input never anchors the menu.
    if (state.narrow && !deps.inline(input)) return;
    ...
    if (tagCtx) {
      if (catalog) { next = { mode: 'tag', ctx: tagCtx }; items = tagChoices(catalog, tagCtx); }
    ...
    if (!next || items.length === 0) { close(); return; }
    menuFor = input;
    menuState = next;
  ...
  function place(): void {
    ...
    if (host) {
      ...
      if (menu.parentElement !== host.container) host.container.insertBefore(menu, host.before);
      return;
    }
    menu.classList.remove('re-menu-inline');
    if (menu.parentElement !== deps.root) deps.root.append(menu);
  ...
  function pick(i: number): boolean {
    const input = menuFor;
    const current = menuState;
    const choice = menuItems[i];
    if (!input || !current || !choice) return false;
    const r = current.mode === 'tag' && (choice.kind === 'device' || choice.kind === 'tag')
      ...
      : current.mode === 'name' && (choice.kind === 'variable' || choice.kind === 'function')
        ...
        : null;
    if (!r || r.text === input.value) return false;
    input.value = r.text;
  ...
  function key(input: HTMLInputElement, e: KeyboardEvent): boolean {
    if (menu.hidden || menuFor !== input) return false;
    switch (e.key) {
      case 'ArrowDown': menuIndex = (menuIndex + 1) % menuItems.length; draw(); break;
      case 'ArrowUp': menuIndex = (menuIndex + menuItems.length - 1) % menuItems.length; draw(); break;
      // A complete name has nothing to pick: the key commits or moves on as usual.
      case 'Enter': case 'Tab': if (!pick(menuIndex)) { close(); return false; } break;
  ...
  let frame = 0;
  const follow = (): void => {
    if (menu.hidden || !menuFor || deps.inline(menuFor)) return;
    cancelAnimationFrame(frame);
    frame = requestAnimationFrame(() => {
      if (document.activeElement !== menuFor) close();
      else place();
    });
  };
