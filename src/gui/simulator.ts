...
interface Node {
  kind: 'condition' | 'variable' | 'tag';
  /** Unique within the tree: the path from the condition down. */
  id: string;
  /** Shown in the label column; a formula is coloured, a name is plain. */
  label: string;
  formula: boolean;
  series: Value[];
  unit?: string;
  /** Dashed lines on an analog lane: the constants conditions compare it with. */
  thresholds: number[];
  children: Node[];
  depth: number;
}
...
function thresholds(rule: Rule): Map<string, number[]> {
  const out = new Map<string, number[]>();
  const add = (key: string, n: number): void => { const list = out.get(key) ?? []; if (!list.includes(n)) list.push(n); out.set(key, list); };
  const keyOf = (n: Ast): string | null => {
  ...
  const walk = (n: Ast): void => {
    if (n.kind === 'binary') {
      if (COMPARISONS.has(n.op)) {
        ...
        if (l && n.right.kind === 'number') add(l, n.right.value);
        if (r && n.left.kind === 'number') add(r, n.left.value);
      }
      walk(n.left); walk(n.right);
    } else if (n.kind === 'call') n.args.forEach(walk);
    else if (n.kind === 'unary') walk(n.arg);
  };
  for (const c of rule.conditions) { const ast = safeParse(c.expr); if (ast) walk(ast); }
  for (const v of rule.variables) { const ast = safeParse(v.formula); if (ast) walk(ast); }
  return out;
}
...
export function initSimulator(root: HTMLElement, opts: SimulatorOptions): SimulatorHandle {
  ...
  const catalog = (): TagCatalog | null => (typeof opts.catalog === 'function' ? opts.catalog() : opts.catalog ?? null);

  /** The catalog entry for a tag, for its unit and its value. */
  const entryOf = (key: string): { unit?: string; value?: string } | null => {
    const cat = catalog();
    if (!cat) return null;
    for (const d of cat.devices) for (const t of d.tags) if (tagKey({ device: d.device, tag: t.tag }) === key) return t;
    return null;
  };

  /** A tag with no signal yet holds what the catalog reads for it. */
  function defaultSignals(): void {
    for (const ref of ruleTags(rule)) {
      const key = tagKey(ref);
      if (state.signals[key] !== undefined) continue;
  ...
  function numberInput(read: () => string, commit: (seconds: number) => void, label: string): HTMLInputElement {
    ...
    input.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') { done(); input.blur(); }
      else if (e.key === 'Escape') { input.value = read(); input.blur(); }
    });
    return input;
  }
  ...
  function renderTags(): void {
    ...
    sim.tags.forEach((series, i) => {
      ...
      input.addEventListener('keydown', (e) => {
        ...
        else if (e.key === 'Escape') { input.value = committed; input.dispatchEvent(new Event('input')); input.blur(); }
      });
      ...
      if (series.error) cell.append(el('p', { class: 'rs-msg' }, [series.error]));
  ...
  function formulaView(text: string): HTMLElement {
  ...
  const valueCells: Array<{ cell: HTMLElement; node: Node }> = [];
  ...
  function buildTree(): Node[] {
    ...
    const unitOf = (ast: Ast | null, seen = new Set<string>()): string | undefined => {
      if (!ast) return undefined;
      if (ast.kind === 'call' && ast.name === 'TAG') { const ref = formulaRefs(ast).tags[0]; return ref ? entryOf(tagKey(ref))?.unit : undefined; }
      if (ast.kind === 'call' && ast.name === 'RATE' && ast.args[0]) { const u = unitOf(ast.args[0], seen); return u ? `${u}/h` : undefined; }
      if (ast.kind === 'call' && ast.name === 'AVG' && ast.args[0]) return unitOf(ast.args[0], seen);
      if (ast.kind === 'ref' && !seen.has(ast.name)) { seen.add(ast.name); return unitOf(varAst.get(ast.name) ?? null, seen); }
      return undefined;
    };
    const children = (ast: Ast | null, parent: string, depth: number, path: Set<string>): Node[] => {
      if (!ast || depth > MAX_DEPTH) return [];
      const refs = formulaRefs(ast);
      const out: Node[] = [];
      for (const name of refs.variables) {
        if (path.has(name)) continue; // a cycle: validation reports it
    ...
    return rule.conditions.map((c, k) => {
      const id = `c:${k}`;
      return {
        kind: 'condition' as const, id, label: c.expr, formula: true, depth: 0,
  ...
  function renderTimeline(): void {
    ...
    const draw = (node: Node, first: boolean): void => {
      ...
      if (node.children.length) label.addEventListener('click', () => { if (closed.has(node.id)) closed.delete(node.id); else closed.add(node.id); renderTimeline(); });
      else label.disabled = true;
      const value = el('span', { class: 'rs-value' });
      valueCells.push({ cell: value, node });
      const numbers = node.series.filter((v): v is number => typeof v === 'number');
      ...
      if (!analog) for (const band of bands(node)) plot.append(band);
      lane.append(label, value, axisCol, plot);
      tree.append(lane);
      if (!closed.has(node.id)) node.children.forEach((c) => draw(c, false));
    };
  ...
  function gridSvg(analog: { numbers: number[]; node: Node; axisCol: HTMLElement } | null, node: Node, ticks: number): SVGElement {
    ...
    for (let k = 1; k < ticks; k++) line((W * k) / ticks, 0, (W * k) / ticks, H, 'rs-grid');
  ...
  function bands(node: Node): HTMLElement[] {
    const n = node.series.length;
    if (n === 0) return [];
    const out: HTMLElement[] = [];
    let start = 0;
    const label = (v: Value): string => formatValue(v, node.unit);
    for (let i = 1; i <= n; i++) {
      if (i < n && label(node.series[i]) === label(node.series[start])) continue;
  ...
  let dragging = false;
  const cursorFromEvent = (e: PointerEvent): void => {
    const plot = tree.querySelector<HTMLElement>('.rs-plot');
    if (!plot) return;
  ...
  tree.addEventListener('pointermove', (e) => { if (dragging) cursorFromEvent(e); });
   ...
   */
  function lead(text: string): (string | HTMLElement)[] {
    const end = text.indexOf('. ');
    if (end < 0) return [el('strong', {}, [text])];
  ...
  const measure = (): void => {
    ...
    if (ticksDrawn && tickCount() !== ticksDrawn) renderTimeline();
  };
...
export function parseSeconds(text: string): number | null {
  ...
  if (t === '') return null;
  if (/^\d+(\.\d+)?s?$/.test(t)) return Number(t.replace(/s$/, ''));
