 ...
 */
type Loc = Pick<ValidationIssue, 'rule' | 'field' | 'variable' | 'condition' | 'action'>;
...
const EDGE_OPTIONS: Opt[] = [
  { value: 'rising', label: 'becomes true' },
  { value: 'none', label: 'is true' },
];
const EDGE_META: Record<Edge, string> = { rising: 'rising edge', none: 'every cycle' };
...
type ThenField = 'topic' | 'payload' | 'source' | 'summary' | 'firstStep' | 'cause';
type ThenRow = { kind: 'publish'; index: number; field: 'topic' | 'payload' } | { kind: 'incident'; field: 'source' | 'summary' | 'firstStep' | 'cause' };

const THEN_LABEL: Record<ThenField, string> = { topic: 'topic', payload: 'payload', source: 'source', summary: 'title', firstStep: 'first step', cause: 'cause' };
/** The `field` name in a ValidationIssue for each Then field. */
const THEN_ISSUE_FIELD: Record<ThenField, ValidationIssue['field']> = {
  topic: 'topic', payload: 'payload', source: 'source', summary: 'summary', firstStep: 'first_step', cause: 'cause',
};

function thenRows(rule: Rule): ThenRow[] {
  const rows: ThenRow[] = [];
...
function thenGet(rule: Rule, row: ThenRow): string {
  if (row.kind === 'publish') return rule.actions[row.index][row.field] ?? '';
  return rule.incident?.[row.field] ?? '';
}

function thenSet(rule: Rule, row: ThenRow, value: string): void {
  if (row.kind === 'publish') {
 ...
 */
function previewThen(text: string, rule: Rule): string | null {
...
const scoped = (cls: string, block: string): string => block.replace(/\.re-root /g, `.re-root.${cls} `);
...
const WIDTH_CLASSES: Array<[string, number]> = [['is-medium', 900], ['is-narrow', 560], ['is-tight', 430]];
const STYLES = `
.re-root {
...
export function initRulesEditor(root: HTMLElement, opts: RulesEditorOptions = {}): RulesEditorHandle {
  ...
  const resolveCatalog = (): TagCatalog | null =>
    typeof catalogSource === 'function' ? catalogSource() : catalogSource ?? null;
  ...
  function messageFor(node: HTMLElement): HTMLElement | null {
    const host = node.classList.contains('re-cell') ? node.parentElement ?? node : node;
    const loc = node.dataset.loc ?? '';
    return Array.from(host.children).find((c) => c.classList.contains('re-msg') && (c as HTMLElement).dataset.for === loc) as HTMLElement | undefined ?? null;
  }

  /** Issue counts and names in the rail, without re-rendering it. */
  function markRail(issues: ValidationIssue[]): void {
    ...
    for (const row of Array.from(rail.querySelectorAll<HTMLElement>('.re-rail-row'))) {
      ...
      if (rule && name) name.textContent = rule.name || 'unnamed';
      const meta = row.querySelector('.re-rail-meta');
      if (rule && meta) meta.textContent = EDGE_META[rule.edge ?? 'none'];
      const square = row.querySelector('.re-square');
      if (rule && square) square.className = `re-square ${rule.incident ? `is-${rule.incident.severity}` : 'is-hollow'}`;
    }
    const select = pane.querySelector<HTMLSelectElement>('.re-rail-select');
    if (select) {
  ...
  // ---- controls ----
  const identifierAttrs = (input: HTMLInputElement): HTMLInputElement => {
  ...
  function textInput(
    ...
    input.addEventListener('input', () => { o.onDraft?.(input.value); if (o.formula) maybeMenu(input); });
    input.addEventListener('change', commit);
    input.addEventListener('keydown', (e) => {
      // An open TAG("…") menu takes the arrows, Enter, Tab and Escape first.
      if (o.formula && menuKey(input, e)) return;
    ...
    if (o.formula) {
      ...
      input.addEventListener('blur', () => { if (menuFor === input) closeMenu(); });
    }
  ...
  function placeMenu(): void {
    ...
    if (menuFor === barInput) {
      ...
      if (menu.parentElement !== bar) bar.insertBefore(menu, barLine);
      return;
    }
  ...
  function menuKey(input: HTMLInputElement, e: KeyboardEvent): boolean {
    if (menu.hidden || menuFor !== input) return false;
    switch (e.key) {
      case 'ArrowDown': menuIndex = (menuIndex + 1) % menuItems.length; drawMenu(); break;
      case 'ArrowUp': menuIndex = (menuIndex + menuItems.length - 1) % menuItems.length; drawMenu(); break;
      case 'Enter': case 'Tab': pickMenu(menuIndex); break;
  ...
  function liveCell(label: string, ref: () => MonitorRef, info: Partial<CellInfo>): HTMLElement {
    const read = (): string | undefined => monitor?.(ref());
  ...
  function renderRail(): void {
    ...
    const filterInput = identifierAttrs(el('input', {
      ...
      oninput: (e) => { filter = (e.target as HTMLInputElement).value; applyFilter(); },
    }));
    ...
    model.rules.forEach((rule, index) => {
      ...
      const row = el('div', {
        ...
        onkeydown: (e) => { if ((e as KeyboardEvent).key === 'Enter') selectRule(index); },
      }, [
        el('span', {}, [square]),
  ...
  function deleteRule(index: number): void {
    ...
    if (selected >= model.rules.length) selected = Math.max(0, model.rules.length - 1);
    render();
  }
  ...
  const barInput = el('input', {
    ...
    oninput: () => {
      ...
      if (info.formula) maybeMenu(barInput);
    },
    onclick: () => { const info = selectedCell ? cellInfo.get(selectedCell) : undefined; if (info?.formula) maybeMenu(barInput); },
    onkeydown: (e) => {
      if (menuKey(barInput, e as KeyboardEvent)) return;
