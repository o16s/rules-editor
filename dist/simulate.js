...
export function parseSignal(text) {
    ...
    if (ast.kind !== 'call') {
        ...
        if (literal(ast) === null)
            throw new Error('A signal is HOLD, STEP, RAMP, PULSE or SINE, or a single value such as 20.');
    ...
    if (ast.args.length < spec.minArgs || ast.args.length > spec.maxArgs)
        throw new Error(`${spec.name} takes ${spec.minArgs} argument${spec.minArgs === 1 ? '' : 's'}: ${spec.signature}.`);
    for (const a of ast.args)
        if (literal(a) === null)
            throw new Error(`${spec.name}: every argument is a fixed value. A signal cannot read a tag or a variable.`);
    const num = (i, what) => { if (typeof literal(ast.args[i]) !== 'number')
        throw new Error(`${spec.name}: ${what} is a number of seconds, or a duration such as 180s.`); };
    if (ast.name === 'STEP')
        num(2, 'the time');
    if (ast.name === 'RAMP') {
        num(0, 'from');
 ...
 */
export function emptySimulation(rule) {
    return {
        times: [0],
 ...
 */
export async function simulate(rule, opts) {
    ...
    const variables = rule.variables.map((v) => ({ name: v.name, text: v.formula }));
    const rows = rule.conditions.map((c) => ({ name: c.expr, text: c.expr }));
    let out;
    try {
        out = await runEngine({
        ...
        return {
            ...
            result: [...blank],
            fires: [],
            log: [{ t: 0, text: `The rule engine did not load: ${e.message}` }],
        };
    }
    const named = (s) => ({ name: s.name, values: s.values });
 ...
 */
function writeLog(rule, times, conditions, out, step) {
    const log = [];
    const LOG_MAX = 200;
    let hidden = 0;
    ...
    if (out.error)
        say({ t: 0, text: out.error });
    for (const line of [...out.variables, ...out.rows]) {
        if (line.problem)
            say({ t: 0, text: `${line.name}: ${line.problem}` });
    }
    ...
    if (hidden)
        log.push({ t: times[times.length - 1] ?? 0, text: `${hidden} more events are not shown.` });
    void step;
    return log;
}
