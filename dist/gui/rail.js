...
const squareClass = (rule) => `re-square ${rule.incident ? `is-${rule.incident.severity}` : 'is-hollow'}`;
export function createRail(deps) {
    const { state } = deps;
    const rail = el('aside', { class: 're-rail', 'aria-label': 'Rules' });
    const rows = () => Array.from(rail.querySelectorAll('.re-rail-row'));
    function render() {
    ...
    function mark(issues) {
        ...
        for (const row of rows()) {
            ...
            if (square)
                square.className = squareClass(rule);
        }
        const select = deps.pane.querySelector('.re-rail-select');
        if (select) {
    ...
    function select(index) {
        if (index === state.selected || !state.model.rules[index])
            return;
    ...
    function remove(index) {
        ...
        if (state.selected >= state.model.rules.length)
            state.selected = Math.max(0, state.model.rules.length - 1);
        deps.render();
    }
